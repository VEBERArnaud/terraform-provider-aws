...
func resourceAwsAmplifyAppCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("custom_headers"); ok {
		params.CustomHeaders = aws.String(v.(string))
	}
	...
	if v, ok := d.GetOk("enable_branch_auto_deletion"); ok {
		params.EnableBranchAutoDeletion = aws.Bool(v.(bool))
	}
	...
	d.SetId(aws.StringValue(resp.App.AppId))
...
func resourceAwsAmplifyAppRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("custom_headers", resp.App.CustomHeaders)
	...
	d.Set("enable_branch_auto_deletion", resp.App.EnableBranchAutoDeletion)
...
func resourceAwsAmplifyAppUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("custom_headers") {
		params.CustomHeaders = aws.String(d.Get("custom_headers").(string))
	}
	...
	if d.HasChange("enable_branch_auto_deletion") {
		params.EnableBranchAutoDeletion = aws.Bool(d.Get("enable_branch_auto_deletion").(bool))
	}
...
func expandAmplifyAutoBranchCreationConfig(v []interface{}) (*amplify.AutoBranchCreationConfig, []*string, *bool) {
	...
	if ev, ok := e["enable_performance_mode"].(bool); ok {
		config.EnablePerformanceMode = aws.Bool(ev)
	}
...
func flattenAmplifyAutoBranchCreationConfig(config *amplify.AutoBranchCreationConfig, patterns []*string, enable *bool) []map[string]interface{} {
	...
	if config != nil {
		...
		value["enable_performance_mode"] = aws.BoolValue(config.EnablePerformanceMode)
