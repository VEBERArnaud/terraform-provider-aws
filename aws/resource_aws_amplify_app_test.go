...
func TestAccAWSAmplifyApp_CustomHeaders(t *testing.T) {
	...
	// once set, build_spec cannot be removed.
	customHeaders1 := "version: 0.1"
	customHeaders2 := "version: 0.2"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccAWSAmplifyAppConfigCustomHeaders(rName, customHeaders2),
				Check: resource.ComposeTestCheckFunc(
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "auto_branch_creation_config.0.enable_performance_mode", "true"),
...
func TestAccAWSAmplifyApp_enableBranchAutoDeletion(t *testing.T) {
	rName := acctest.RandomWithPrefix("tf-acc-test")
	resourceName := "aws_amplify_app.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSAmplifyAppConfigEnableBranchAutoDeletion(rName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "enable_branch_auto_deletion", "true"),
				),
			},
			{
				ResourceName:      resourceName,
...
func testAccAWSAmplifyAppConfigCustomHeaders(rName string, customHeaders string) string {
	return fmt.Sprintf(`
resource "aws_amplify_app" "test" {
  name = "%s"
...
func testAccAWSAmplifyAppConfigEnableBranchAutoDeletion(rName string) string {
	return fmt.Sprintf(`
resource "aws_amplify_app" "test" {
  name = "%s"
