...
func resourceAwsAmplifyDomainAssociation() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"auto_sub_domain_creation_patterns": { // NEW
				Type:     schema.TypeList,
				Optional: true,
				MinItems: 1,
				Elem:     &schema.Schema{Type: schema.TypeString},
			},
			"auto_sub_domain_iam_role": { // NEW
				Type:     schema.TypeString,
				Optional: true,
			},
			"domain_name": {
...
func resourceAwsAmplifyDomainAssociationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("auto_sub_domain_creation_patterns"); ok && len(v.([]interface{})) > 0 {
		params.AutoSubDomainCreationPatterns = expandStringList(v.([]interface{}))
	}

	if v, ok := d.GetOk("auto_sub_domain_iam_role"); ok {
		params.AutoSubDomainIAMRole = aws.String(v.(string))
	}
...
func resourceAwsAmplifyDomainAssociationRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("auto_sub_domain_creation_patterns", resp.DomainAssociation.AutoSubDomainCreationPatterns)
	d.Set("auto_sub_domain_iam_role", resp.DomainAssociation.AutoSubDomainIAMRole)
...
func resourceAwsAmplifyDomainAssociationUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("auto_sub_domain_creation_patterns") {
		params.AutoSubDomainCreationPatterns = expandStringList(d.Get("auto_sub_domain_creation_patterns").([]interface{}))
	}

	if d.HasChange("auto_sub_domain_iam_role") {
		params.AutoSubDomainIAMRole = aws.String(d.Get("auto_sub_domain_iam_role").(string))
	}
