...
* `custom_headers` - (Optional) Custom HTTP headers for the Amplify App.
...
* `enable_branch_auto_deletion` - (Optional) Enables auto-deletion of branches for the Amplify App.
...
* `enable_performance_mode` - (Optional) Enables performance mode (faster hosting) for auto created branch.
